...
q='''query($code:String!,$fightIDs:[Int]){reportData{report(code:$code){fights(fightIDs:$fightIDs){startTime endTime} playerDetails(fightIDs:$fightIDs,includeCombatantInfo:true)}}}'''
res=gql(q,{'code':code,'fightIDs':[fight]})
fi=res['data']['reportData']['report']['fights'][0]
q2='''query($code:String!,$fightIDs:[Int],$start:Float,$end:Float){reportData{report(code:$code){events(fightIDs:$fightIDs,startTime:$start,endTime:$end,useAbilityIDs:true){data}}}}'''
ev=gql(q2,{'code':code,'fightIDs':[fight],'start':fi['startTime'],'end':fi['startTime']+5000})['data']['reportData']['report']['events']['data']
print('events',len(ev))
for e in ev[:20]:
